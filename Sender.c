...
int main(){
    ...
    if(connect(sender_sockfd, (struct sockaddr *)&receiverAddress, sizeof(receiverAddress)) < 0){
    ...
    while(loop!='Y'||loop!='y'){
        ...
        char buff[BUFF_SIZE]={0}; //buffer var
        char buffcc[256]={0};
...
/*            if(byteSent==0){
        ...
        if(loop!='Y'||loop!='y'){
            ...
            int lenEx = strlen(ctumsg),byteSent=0;
...
/*                if(byteSent==0){
    ...
    int lenEx = strlen(exitmsg),byteSent=0;
